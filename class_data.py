...
class ClassData:
    def __init__(self):
        ...
        for id in range(Hyper.num_classes):
            ...
            self.pred_match_dict[id] = PredMatchBuffer()
    ...
    def add_matches(self, buffer, gt_match, pred_match):
        ...
        for i in range(Hyper.num_classes):
            ...
            if len(class_pred_match_dict[i].match) > 0:
                self.pred_match_dict[i].add(class_pred_match_dict[i].match, class_pred_match_dict[i].score)
    ...
    def split_matches_from_image_into_class(self, buffer, gt_match, pred_match):
        ...
        for id in range(Hyper.num_classes):
            ...
            class_pred_match_dict[id] = PredMatchBuffer()
        ...
        for pred_m in pred_match:
            i += 1
            pred_m = int(pred_m.item())
            pred_class_id = buffer.pred_class_ids[i]
            pred_score = buffer.pred_scores[i]
            class_pred_match_dict[pred_class_id.item()].add(pred_m, pred_score)
